...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="writeCssFile">
		<xsl:result-document href="{testng:absolutePath('style.css')}"
            ...
            <![CDATA[
            body { font-family: "Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif; font-size: 12px; padding: 10px; margin: 0px; background-color: white; }
	...
	<xsl:template name="htmlHead">
		<head>
			<title>
				<xsl:value-of
					select="testng:getVariableSafe($testNgXslt.reportTitle, 'Test Results')" />
			</title>
	...
	<xsl:template name="powered-by">
		<div
			...
			<a href="http://isiom-unitbv.pentalog.ro/referentiel/" style="color: #8888aa;"
				target="_blank">
